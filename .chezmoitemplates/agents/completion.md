...
Choose checks that cover the changed behavior and material risks, with validation effort proportional to the decision and the user's stated standard. Map every behavior, owner, or loading change to a check that distinguishes the relevant old and new semantics. Do not substitute repeated checks, a mechanical assembly pass, or a weaker proxy for completing the task, semantic conformance, or loading proof.
