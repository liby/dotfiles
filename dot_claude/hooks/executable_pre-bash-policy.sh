...
# (grep muscle memory: -rn, -nr, -rln), and --include is grep-only. Fail-open
# by design: the whole command is one record so quote state spans newlines,
# detection arms only at a command-position rg, and disarms at separators,
# --, and -e operands. Quoted mentions (commit messages), other commands'
# flags (sort -rn), and heredoc bodies (stripped by parse_command) pass.
