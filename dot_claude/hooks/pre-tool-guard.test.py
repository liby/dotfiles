...
class GuardTests(unittest.TestCase):
    ...
    def test_command_policy(self):
        cases = [
            ...
            (2, 'nohup find . -name x > /tmp/dev.log 2>&1 &'),
            (2, 'nohup -- find . -name y'),
            (2, 'env NODE_ENV=development nohup find . -name z'),
            (0, './scripts/exec find . -name x'),
            ...
            (0, 'echo done; # find . -name x'),
            (0, "rg pattern . # 'a\\|b' --include=x -rn"),
            (2, "echo done # don't edit\nrg -rn pattern ."),
            (2, 'echo file\\ #literal; find . -name x'),
            (2, 'echo file\\)#literal; find . -name y'),
            (2, 'echo ""#literal; find . -name z'),
            (2, "echo ''#literal; find . -name x"),
            (2, 'echo "#literal"; find . -name x'),
            (2, 'echo \\#literal; find . -name x'),
            (2, 'echo file\\\n#literal; find . -name x'),
            (0, 'echo file \\\n# find . -name x'),
            ...
            (2, 'env "FOO=$BAR" find . -name x'),
            (2, 'env FOO="$BAR" find . -name x'),
            (0, 'env "${NAME}"=value find . -name x'),
            (0, 'git commit -m "fix; find bug"'),
            (2, '> output find . -name x'),
            (2, 'nohup >output 2>&1 find . -name x'),
            (2, "rg '-rn' x ."),
            (0, "printf '%s' '|' find . -name x"),
            (2, "printf '%s' '|' ; find . -name x"),
            (0, "printf '%s' '{' find . -name x"),
