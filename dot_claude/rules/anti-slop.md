...
- **Use literal editing verbs.** For file moves, renames, deletions, or structural edits, use `移 / 改名 / 删 / 加`. Do not use `砍` / `砍除` to mean deleting edits, files, or code; do not flag the token outside an edit context.
...
- **End the sentence at the actual point.** Delete redundant trailing soft-restatements (`这说明……`, `也就是说……`, `换句话说……`, `可以看出……`, `In other words…`). If the previous sentence already made the point, stop there.
...
- Trailing restatement: `这说明`, `也就是说`, `换句话说`, `可以看出`, `In other words`
- Corporate jargon: `落地`, `落库`, `落盘`, `闭环`, `开干`, `起飞`, `抓手`, `赋能`, `更硬`, `最硬`
- Edit-deletion metaphor: edit-context `砍` / `砍除` (use `删` / `删除`)
