...
- Use concrete action verbs in subject and body: `reject empty subscriber list`, `validate write access before subscribing`, `cut p99 from 200ms to 50ms`. Vague AI verbs like `tighten`, `streamline`, `enhance`, `refine`, `polish`, bare `optimize` (no metric), or bare `harden` (no named threat) leave readers guessing what changed.
