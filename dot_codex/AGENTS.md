...
- NEVER read, create, overwrite, or copy secret plaintext or credential-store contents, including real `.env*` files and private keys, even when requested. Hand operations that can reveal or derive plaintext to the user. The only plaintext exceptions are a placeholder-only `.env.example` template and disposable non-secret fixtures created inside an isolated probe home under the rule below; neither may contain or derive from real credentials.
