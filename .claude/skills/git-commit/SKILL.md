...
3. Stage relevant files:
    ...
    - `dot`: use `dot add -u` by default; `dot add -f <file>` for new files (`core.excludesFile` ignores `*`); NEVER `dot add -A` (work tree is $HOME, would stage everything). Always use **absolute paths** — relative paths resolve from CWD, not from work-tree root
