...
- `chezmoi:*` and `docker:*` are host-integration exceptions: chezmoi manages paths across the home directory, and Docker controls the host daemon. `git:*` keeps Git on the regular auto-mode decision path instead of sandbox auto-allow.
- Extra write paths are limited to development tool caches and stores. Add a path only after an observed sandbox denial, and do not widen access to a whole home or source tree.
...
- Allowed domains cover the source hosts, package registries, and Claude endpoints used by local development. Add domains from observed traffic, not anticipated convenience.
