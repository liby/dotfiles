...
- Treat `excludedCommands` entries as deliberate command shapes, not universal wrapper-aware policies. Matching runs per segment and exempts the invocation only when every segment starts with an excluded command; a newline separates segments just as `&&`, `;`, and `|` do, so a multi-line command counts as several. One ordinary command anywhere in the invocation, including a trailing `| jq`, an `&& echo`, or a `test` on the preceding line, sandboxes all of it, and a socket-based tool such as `herdr` then fails with `Operation not permitted` rather than a recognizable permission error. A segment must start with the excluded command itself: a leading `cd`, variable assignment, `command`, or `env` keeps it sandboxed, and so do Git options that select another repository or configuration, such as `-C`, `-c`, `--git-dir`, and `--work-tree`, which a built-in check rejects before any pattern is compared, so no entry form such as `git *` can exempt them; `git --no-pager` stays exempt. Do not broaden a matcher without an observed failure.
