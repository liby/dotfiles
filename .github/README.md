...
此外，bare repo 通过 `core.excludesFile` 指向 `~/.config/git/dotfiles-ignore`（内容为通配符 `*`），用于忽略所有未追踪的文件。这可以防止 `dot status -u` 扫描整个 `$HOME` 目录（避免 macOS 隐私权限警告），同时也杜绝了意外追踪文件的可能。因此，添加新文件时需要使用 `-f` 参数强制添加：
...
```sh
dot add -f <file>：添加新文件到仓库（需要 -f 因为 core.excludesFile 忽略了所有文件）
