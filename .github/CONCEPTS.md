...
Script identity includes rendered comments and other non-executable bytes. A comment-only edit to a `run_once` or `run_onchange` source can therefore schedule the script again, and a `Brewfile` comment changes the explicit hash embedded in its installer. A `run_onchange` script is keyed by its target name as well, so renumbering one reschedules it with its body unchanged. Treat those comments as operational input and edit them only when the resulting rerun is in scope.

These dependencies shape the sequence:

- Homebrew and [`Brewfile`](../Brewfile) packages provide tools used by later scripts.
- proto resolves every tool version from `~/.proto/.prototools`, which chezmoi writes from [`dot_proto/dot_prototools`](../dot_proto/dot_prototools) during file synchronization. The proto and dev-tools scripts therefore run in the `after` phase: in the `before` phase that file does not exist yet, and proto refuses to install a tool whose required Node.js version is unconfigured.
...
The [Codex requirements script](../.chezmoiscripts/run_onchange_after_08-setup-codex-requirements.sh.tmpl) writes the rendered requirements to the macOS preference independently of Apple UI defaults. Keep these lifecycle owners separate so a hook registration or timeout change does not restart ControlCenter, Dock, or Finder.
...
[`git-ssh-gpg-agent`](../dot_config/git/executable_git-ssh-gpg-agent) exists because gpg-agent serves the card's SSH key on `~/.gnupg/S.gpg-agent.ssh` while shells keep the macOS ssh-agent in `SSH_AUTH_SOCK`. The wrapper points `SSH_AUTH_SOCK` at the gpg-agent socket, then runs `ssh` for `core.sshCommand --transport` or `ssh-keygen` as `gpg.ssh.program`, so Git transport and SSH signing reach the YubiKey from any shell, including sandboxed agent shells that inherit no agent environment. Bare `ssh` keeps the macOS agent and file keys, except on `github.com`, where `IdentityAgent` names the gpg-agent socket so the card's authentication key is reachable without Git. Changing that socket means updating both that entry and the wrapper.

### Credential-backed features

The [seeding script](../.chezmoiscripts/run_onchange_after_06-seed-envchain.sh.tmpl) reads `.secrets/seed.asc` and stores each entry in the macOS Keychain through envchain. Its plaintext is TOML with one top-level table per namespace and one environment variable per single-line string value; numeric and boolean-looking values remain quoted. The template below is the machine-checked contract for entries consumed by files in this repository; other seed entries are not repository requirements.
