...
- Before changing a fragment applied through a paired `modify_` template, read that template and any path-scoped rule covering either file. Removing a value from the fragment stops managing it but leaves the existing destination value unchanged; retire it once by removing it from both the fragment and destination file. Add repeatable deletion to the `modify_` template only when the value must remain absent on every render.
