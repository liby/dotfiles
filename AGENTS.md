...
- **Codex App write-backs**: Enable Computer Use only through `plugins."computer-use@openai-bundled"` and leave App-owned `features.js_repl`, `mcp_servers.computer-use`, `mcp_servers.node_repl.env`, and `model_reasoning_effort` unmanaged. Keep `model`, `desktop.followUpQueueMode = "steer"`, and the portable `mcp_servers.node_repl` launch fields managed. The App restores `js_repl` and `node_repl` on startup, so ignore the removed feature flag while preserving App-written runtime fields; see [openai/codex#24387](https://github.com/openai/codex/issues/24387#issuecomment-4531286877).
