...
- **Codex filesystem blacklist**: Keep the custom `development` profile standalone with `:root = write` and explicit credential and workspace environment-file denies. The managed requirements allowlist exposes only `development` and built-in `:danger-full-access`; `:workspace` and `:read-only` stay unavailable, while `development` remains the default. Do not extend `:workspace`, mirror per-tool cache allowlists, or add command `allow` rules for routine workflows: inherited protected paths recreate special cases, while command rules execute outside the sandbox and bypass filesystem denies. Filesystem capability does not authorize state-changing Git or secret-bearing chezmoi operations.
