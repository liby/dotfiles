...
If a query emits an SSO URL instead of opening a browser, keep that query running and use Browser or Chrome control to open the exact URL in a connected signed-in browser; that browser must already hold the identity-provider session, so if the opened page is a sign-in or MFA prompt, terminate the query and report that instead of waiting out the timeout. Keep the URL out of commentary, final responses, and other shell processes. When output is explicitly truncated or incomplete, recover the full original URL through a supported interface for the same running query; never reconstruct it or substitute another query's URL. If recovery is unavailable, terminate the query and report that boundary. Reapproval does not repair an incomplete payload. If browser control is unavailable or navigation fails, terminate the query and report that boundary. After navigation, wait up to two minutes for the original query; its exit status is authoritative even if the browser reports a callback-page error. If it times out, terminate it and report that the SSO callback did not complete.
