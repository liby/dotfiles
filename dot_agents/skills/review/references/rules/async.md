...
When changed code participates in a lifecycle containing at least two of scheduling, retries, deduplication, or external expiry, compare them as one lifecycle even if only one leg changed: attempt cadence, dedupe horizon and identity, state change on failure, retry terminal state, and external authority lifetime. A failed attempt must not suppress the next required attempt through unchanged durable state, a memoized result, or reused duplicate identity.
