...
Do not edit behavior-bearing agent instructions, including skills, output styles, `AGENTS.md` or `CLAUDE.md`, agent rules or prompts, and examples or explanatory prose within those surfaces. On mixed changes, leave those bytes to `write-skill` and clean only the code, tests, code comments, and ordinary project documentation; report any instruction finding under `Candidates left` with `write-skill` as its owner.
...
2. **Classify before mutation.** The invoking session is the sole writer and owns every artifact disposition. When independent review pays for coordination and agents are available, assign read-only reviewers to the quality lenses below; the Complexity reviewer also cross-checks artifact value. If an assigned reviewer fails, cover its lens locally and record `independent quality review unavailable` under `Candidates left`. Put all findings in the same frontier before mutation.
   ...
   - **Artifact value:** A current-change artifact is newly added, substantively modified, or made directly redundant by this change; whole-file scope alone does not qualify. Give each an evidence-backed keep, rewrite, delete, or candidate disposition. Historical reachability alone is not independent value. After the owning mechanism changes, re-establish the artifact's trigger in current callers, control flow, runtime evidence, or an external contract; otherwise delete a test that only proves a retired failure stays absent, its unreachable guard or fallback, and comments explaining either. Delete any other artifact with no independent value after checking its available owner; reserve candidate for a concrete unavailable owner.
     ...
     - **Code comments and ordinary project documentation:** Establish what a maintainer or reader needs this text to explain, then check its claims against the owning requirements, documentation, fixtures and issue context. Preserve verified rationale and constraints at the boundary where they affect a decision; documentation elsewhere does not make that local explanation redundant. Remove implementation mirrors and obsolete or unsupported claims when they have no independent value. Rewrite a surviving block coherently, preserving its language and technical meaning, instead of appending a corrective sentence to contradictory prose. Do not preserve intermediate attempts or speculative future work. Keep tool directives as code, use names and direct flow for implementation structure, and apply the repository's language policy to new prose.
...
5. **Validate the final state.** Discover the cheapest relevant validation from repository instructions, package or task configuration, build files, and adjacent tests. Run it after the last mutation, scoped to changed files when supported; any later validation-relevant edit invalidates the result. If validation fails, reopen the frontier for failures attributable to this workflow, fix them, and rerun validation. Do not label a failure pre-existing or unattributed without evidence; if attribution requires unavailable external state, record the exact gap under `Candidates left` and stop without unrelated fixes. Run a dev, start, or serve command only when it takes no resource from an in-use instance, and stop it once it is no longer needed; otherwise request the exact runtime observation. If no command exists, name the sources searched.

   Done when validation passes on the final state or a precise external, pre-existing, or manual gap is recorded.

6. **Reconcile and report.** Re-run `git status --short` and compare it with the initial scope plus this workflow's edits. Unexpected new or missing paths remain explicit candidates; do not reconcile them silently.

   Report the final state with these labels:

   - `Changed:` each edited file and its reason, or `none`. For a deleted or rewritten comment, include the pre-edit `path:line` and removed text or `before -> after` pair. Describe secret-bearing material without quoting it.
   - `Retained:` include this only when a current-change test, comment or documentation block has a keep or rewrite disposition. Group survivors by the distinct behavior, contract or rationale they protect, naming the production path and mock boundary when relevant.
   - `Candidates left:` unresolved ownership, evidence, scope or stop gaps, including candidate survivors, or `none`. Explain the missing observation or authority without converting it into speculative work for the reader.
   - `Validation:` the relevant command and result, or the reason it was skipped.

   Report only after the last validation-relevant mutation, with every label describing the final working tree.
