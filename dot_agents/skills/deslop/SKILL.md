...
2. **Classify before mutation.** The invoking session is the sole writer and owns every artifact disposition. When independent review pays for coordination and agents are available, assign read-only reviewers to the quality lenses below; the Complexity reviewer also cross-checks artifact value. If an assigned reviewer fails, cover its lens locally and record `independent quality review unavailable` under `Candidates left`. Put all findings in the same frontier before mutation.
   ...
   - **Artifact value:** A current-change artifact is newly added, substantively modified, or made directly redundant by this change; whole-file scope alone does not qualify. Give each an evidence-backed keep, rewrite, delete, or candidate disposition. After inspecting available owners, delete one with no independent value; reserve candidate for a concrete unavailable external owner.
     - **Tests:** Trace every current-change test through production logic and name the distinct observable fault or contract it detects and its mock boundary. Exact values and external mocks count when they carry that signal; a source or declarative shape counts only when it is itself an external contract or is exercised by a real consumer or renderer. Tests of mocks and coverage-only tests do not. Merge a subset or matrix row only when a survivor covers the same production path, input partition, observations, and failure modes. Before rewriting or deleting, map every assertion and partition to retained coverage, a survivor, an established non-contract, or a candidate. In a mixed test remove only brittle detail; do not add a seam solely for coverage. Delete only zero marginal observable fault or contract detection.
     - **Comments and documentation:** Inspect non-obvious current-change decisions, including uncommented ones, and search governing requirements, documentation, fixtures, and issue context before calling a rationale unsupported. Clarify structure through names, types, or direct control flow; do not add a helper solely to remove a comment. Classify each comment or prose documentation block, then apply the matching action:
       - A verified why or external constraint not inferable from code: keep its shortest form; when kept comments would repeat it, keep only the copy at the owning boundary. Being documented elsewhere does not disqualify a causal constraint; the pointer rule below bounds the detail.
       - A restating or implementation-mirroring comment where that search surfaced a cause or constraint whose omission invites harmful simplification: rewrite it into that shortest verified why.
       - Any other restating or implementation-mirroring comment: delete it. A census of current state, such as counts, caller inventories, or who-uses-this lists, is restating even when accurate.
       - A claim that neither that search nor the code's own observable behavior supports, or a rationale another kept comment already states: delete it.
       - A contrastive aside naming something absent from the current code ("unlike X", "rather than Y", "no longer Z"): excise the clause when the contrast carries no live constraint; a contrast encoding why the absent alternative fails is a verified why and stays.
       - Commented-out code: an ordinary artifact disposition; delete it only with current-change ownership and no independent value, otherwise report it under `Candidates left`.
       - An audience-facing documentation block or doc comment stating an observable contract or durable information: keep it, tightened; it is not an implementation mirror.
       - Any other inline prose comment: not a verified why; delete it. Tool directives and pragmas are code, not comments.

       Keep survivors in wording a non-native reader parses once: everyday words, one idea per sentence, domain terms intact. Keep a pre-existing comment's original language; hold current-change survivors and rewrites to the repository's comment-language policy. The owning source carries detail; a pointer only supports the why. Audience documentation states observable contracts or durable information, not implementation mirrors.
