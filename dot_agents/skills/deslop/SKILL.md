...
2. **Classify before mutation.** The invoking session is the sole writer and owns every artifact disposition. When independent review pays for coordination and agents are available, assign read-only reviewers to the quality lenses below; the Complexity reviewer also cross-checks artifact value. If an assigned reviewer fails, cover its lens locally and record `independent quality review unavailable` under `Candidates left`. Put all findings in the same frontier before mutation.
   ...
   - **Artifact value:** A current-change artifact is newly added, substantively modified, or made directly redundant by this change; whole-file scope alone does not qualify. Give each an evidence-backed keep, rewrite, delete, or candidate disposition. After inspecting available owners, delete one with no independent value; reserve candidate for a concrete unavailable external owner.
     ...
     - **Comments and documentation:** Inspect non-obvious current-change decisions, including uncommented ones, and search governing requirements, documentation, fixtures, and issue context before calling a rationale unsupported. Clarify structure through names, types, or direct control flow; do not add a helper solely to remove a comment. Keep only the shortest verified why or external constraint not inferable from code. Rewrite a restating or implementation-mirroring comment only when that evidence supplies such a cause or constraint whose omission invites harmful simplification; otherwise delete it with unsupported claims and duplicate rationale. The owning source carries detail; a pointer only supports the why. Audience documentation states observable contracts or durable information, not implementation mirrors.
